...
class OdooPurchaseOrder(models.Model):
    ...
    odoo_id = fields.Many2one(
        comodel_name="purchase.order", string="Purchase Order", required=True, ondelete="cascade"
    )
    ...
    def sync_order_lines(self):
        for binding in self:
            ...
            for remote_line_id in remote_line_ids:
                ...
                job_info = line_model.import_record(
                    binding.backend_id, remote_line_id, force=True
                )
                if binding.backend_id.delayed_import_lines:
                    job = self.env["queue.job"].search(
                        [("uuid", "=", job_info.uuid)], limit=1
                    )
                    if job:
                        delayed_line_ids.append(job.id)
...
class OdooPurchaseOrderLine(models.Model):
    ...
    odoo_id = fields.Many2one(
        comodel_name="purchase.order.line", string="Purchase Order Line", required=True, ondelete="cascade"
    )
