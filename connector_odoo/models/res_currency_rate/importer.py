...
class CurrencyImporter(Component):
    ...
    def _init_import(self, binding, external_id):
        rate_ids = self.backend_adapter.search([("currency_id", "=", external_id)]) or []
        # Keep newest rates first without relying on backend-specific search kwargs.
        rate_ids = sorted(rate_ids, reverse=True)
        total = len(rate_ids)
        _logger.info(
